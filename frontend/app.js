...
processBtn.onclick = async () => {
    ...
    try {
        ...
        pitchDeckFormData.append('company', company);
        ...
        if (!response.ok || data.status === "error") {
            ...
            console.log('✅ Pitch deck processed:', data);
            
            // PROFESSIONAL UPGRADE: Ensure the analysis section is VISIBLE
            ...
            if (data.pitch_deck) {
                ...
                console.error('No pitch_deck data in response', data);
            }
        }
    } catch (err) {
...
function renderPitchDeckAnalysis(pitchDeck) {
    ...
    if (pitchDeck.analysis) {
        ...
        let formattedAnalysis = pitchDeck.analysis
            .replace(/^### (.*$)/gim, '<h3 style="color: #fff; border-left: 4px solid var(--primary); padding-left: 0.8rem; font-size: 1.1rem; margin-top: 1.8rem; margin-bottom: 1rem;">$1</h3>')
            .replace(/^## (.*$)/gim, '<h2 style="color: #fff; font-size: 1.6rem; font-weight: 700; margin-top: 2.2rem; margin-bottom: 1.2rem; border-bottom: 1px solid var(--border); padding-bottom: 0.5rem;">$1</h2>')
            .replace(/^# (.*$)/gim, '<h1 style="color: #fff; font-size: 2.2rem; font-weight: 800; margin-bottom: 1.8rem; background: linear-gradient(90deg, #fff, var(--primary)); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">$1</h1>')
    ...
    // PROFESSIONAL UPGRADE: Add Re-Analyze button if it's a warning or partial success
    const isWarning = pitchDeck.status === 'warning' || (pitchDeck.analysis && pitchDeck.analysis.includes('Extraction Warning'));
    if (isWarning && pitchDeck.db_id) {
        analysisHtml += `
            <div style="margin-top: 2rem; padding: 1.5rem; border: 1px dashed var(--secondary); border-radius: 8px; background: rgba(117, 13, 65, 0.1); text-align: center;">
                <p style="color: #fff; margin-bottom: 1rem;">This report was generated during a fallback period.</p>
                <button onclick="reprocessPitchDeck(${pitchDeck.db_id})" style="background: var(--secondary); color: white; border: none; padding: 0.8rem 1.5rem; border-radius: 4px; cursor: pointer; font-weight: 600;">
                    <i data-lucide="refresh-cw" style="width: 16px; height: 16px; display: inline-block; vertical-align: middle; margin-right: 0.5rem;"></i>
                    Force High-Fidelity Re-Analysis
                </button>
            </div>
        `;
    }
    
    // Re-initialize icons in the dynamic content
    ...
    const metrics_src = pitchDeck.key_metrics || {};
    const revenue = metrics_src.revenue || null;
    const growth = metrics_src.growth || metrics_src.growth_rate || null;
    const users = metrics_src.users || null;
    const tam = metrics_src.market || metrics_src.market_size || null;
    const raising = metrics_src.raising || null;
    const runway = metrics_src.runway || null;
...
function renderGrowthTrendChart(pitchDeck) {
    ...
    if (trend.status !== "ok" || !trend.points || trend.points.length === 0) {
        chartContainer.innerHTML = `
            <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; width: 100%; border: 1px dashed rgba(255,255,255,0.1); border-radius: 12px; background: rgba(255,255,255,0.02);">
                <i data-lucide="bar-chart-2" style="width: 32px; height: 32px; color: var(--text-muted); margin-bottom: 0.5rem; opacity: 0.3;"></i>
                <p style="color: var(--text-muted); font-size: 0.9rem;">No reliable revenue growth data identified</p>
                <p style="color: var(--text-muted); font-size: 0.75rem; margin-top: 0.3rem;">Upload detailed financial model for visualization</p>
            </div>
        `;
        if (window.lucide) window.lucide.createIcons();
        return;
    ...
    if (validPoints.length < 3) {
        chartContainer.innerHTML = `
            <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; width: 100%; border: 1px dashed rgba(255,255,255,0.1); border-radius: 12px; background: rgba(255,255,255,0.02);">
                <i data-lucide="bar-chart-2" style="width: 32px; height: 32px; color: var(--text-muted); margin-bottom: 0.5rem; opacity: 0.3;"></i>
                <p style="color: var(--text-muted); font-size: 0.9rem;">Insufficient data points for growth visualization</p>
                <p style="color: var(--text-muted); font-size: 0.75rem; margin-top: 0.3rem;">Need at least 3 historical data points</p>
            </div>
        `;
        if (window.lucide) window.lucide.createIcons();
        return;
...
async function downloadPitchDeck(pitchDeckId) {
    try {
        ...
        const url = window.URL.createObjectURL(blob);// Reprocess Pitch Deck for High-Fidelity Analysis
async function reprocessPitchDeck(pitchDeckId) {
    const analysisBox = document.getElementById('analysis-markdown');
    const oldHtml = analysisBox.innerHTML;
    
    // Show high-tech loading state
    analysisBox.innerHTML = `
        <div style="padding: 3rem; text-align: center;">
            <div class="loader" style="margin: 0 auto 1.5rem auto; border: 4px solid rgba(255,255,255,0.1); border-top: 4px solid var(--secondary); border-radius: 50%; width: 40px; height: 40px; animation: spin 1s linear infinite;"></div>
            <h3 style="color: #fff;">Upgrading to High-Fidelity Analysis...</h3>
            <p style="color: var(--text-muted);">Re-running VC-Grade Hybrid RAG Ingestion</p>
        </div>
    `;
    
    try {
        const response = await fetch(`${API_URL}/pitch-decks/${pitchDeckId}/reprocess`, {
            method: 'POST',
            headers: { 'X-API-KEY': 'finrag_at_2026' }
        });
        ...
        if (data.status === 'success' && data.pitch_deck) {
            ...
            alert('Reprocessing failed to yield a successful result.');
            analysisBox.innerHTML = oldHtml;
        }
    } catch (err) {
